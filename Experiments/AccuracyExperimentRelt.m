...
for i=1:numTrials
    foo = StaticER(G{1},G,1e-8,0.1);
	static_er(i,:) = abs(exact_er(:) - foo(:))./foo(:);
	queryFun = QueryER(G,1e-8,0.1);
    foo = queryFun(G{1}(:,1),G{1}(:,2));
	query_er(i,:) = abs(exact_er(:) - foo(:))./foo(:);
end
...
results = [static_er_mean;
	...
	query_er_mean;
	query_er_std];
fid = fopen('AccuracyResutls10relt.txt','w');
fprintf(fid,'%e %e %e %e\n',results(:));
fclose(fid);
...
results = [static_er_mean;
	...
	query_er_mean;
	query_er_std];
fid = fopen('AccuracyResutls100relt.txt','w');
fprintf(fid,'%e %e %e %e\n',results(:));
fclose(fid);
