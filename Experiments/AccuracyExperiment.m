...
for i=1:numTrials
	static_er(i,:) = StaticER(G{1},G,1e-8,0.1);
	queryFun = QueryER(G,1e-8,0.1);
	query_er(i,:) = queryFun(G{1}(:,1),G{1}(:,2));
end
...
results = [static_er_mean;
	...
	query_er_std;
	exact_er'];
fid = fopen('AccuracyResutls10.txt','w');
...
for i=1:numTrials
	static_er(i,:) = StaticER(G{1},G,1e-8,0.01);
...
results = [static_er_mean;
	...
	query_er_std;
	exact_er'];
fid = fopen('AccuracyResutls100.txt','w');
fprintf(fid,'%e %e %e %e %e\n',results(:));
fclose(fid);
